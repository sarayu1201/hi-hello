...
def check():
    ...
    try:
        ...
        # Query using the correct SBI Clerk Test 3 identifiers
        q = questions_col.find_one({
            "source_file": "sbi_clerk_test_3.json",
            "question_number": 72
        })
        if q:
            ...
            print("Question 72 not found in DB with that filter. Let's find any documents matching ID 72 in Test 3:")
            docs = list(questions_col.find({
                "source_file": "sbi_clerk_test_3.json",
                "id": 72
            }))
            for d in docs:
                print(f"  Found ID 72: q={repr(d.get('q')[:60])}, question_image={repr(d.get('question_image'))}")
