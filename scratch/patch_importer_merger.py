...
def patch():
    ...
    # Define our new helper function
    merger_func_code = """
...
"""
    ...
    # Locate def to_latex(text):
    start_idx = content.find("def to_latex(text):")
    if start_idx == -1:
        print("Error: def to_latex not found")
        return
        
    # We will insert the helper function code right before def to_latex(text):
    # And we will insert `text = merge_adjacent_math_blocks(text)` as the first line of to_latex
    
    body_start = content.find("if not text:", start_idx)
    if body_start == -1:
        print("Error: if not text inside to_latex not found")
        return
        
    # Update to_latex body to call merge_adjacent_math_blocks(text)
    patched_content = (
        content[:start_idx] + 
        merger_func_code + 
        "\n\n" + 
    ...
    print("SUCCESS: import_all_papers.py updated with helper merger function and math cleaning logic!")
