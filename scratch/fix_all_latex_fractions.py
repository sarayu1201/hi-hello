...
def fix_latex_fractions():
    ...
    # Matches \frac{A}{B$ where A and B can be numbers or simple variables, and B is followed immediately by $
    pattern = re.compile(r'\\frac\{([a-zA-Z0-9\.\+\-]+)\}\{([a-zA-Z0-9\.\+\-]+)\$')
    ...
    for file in sorted(os.listdir(json_dir)):
        ...
        for q in data:
            ...
            # 1. Fix question text
            new_q = pattern.sub(r'\\frac{\1}{\2}$', question_text)
            ...
            # 2. Fix explanation
            new_exp = pattern.sub(r'\\frac{\1}{\2}$', explanation)
            ...
            for opt_idx, opt in enumerate(options):
                ...
                new_opt = pattern.sub(r'\\frac{\1}{\2}$', opt_text)
                if new_opt != opt_text:
