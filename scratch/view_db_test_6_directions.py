...
def check():
    ...
    try:
        ...
        for q_num in range(90, 95):
            q = questions_col.find_one({"source_file": "sbi_clerk_test_6.json", "question_number": q_num})
            if q:
                print(f"Q{q_num} Direction in DB: {repr(q.get('direction'))}")
            else:
