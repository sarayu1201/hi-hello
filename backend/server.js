...
const seedQuestionBank = async () => {
  try {
    ...
    if (qCount > 0) {
      console.log(`[Seeding] Question pool already has ${qCount} questions. Skipping automatic seeder.`);
      return;
    }
    
    console.log("[Seeding] Question pool is empty. Starting automatic import from QuestionBank/json...");
    const questionBankDir = path.join(__dirname, "..", "QuestionBank", "json");
    if (!fs.existsSync(questionBankDir)) {
      console.warn(`[Seeding] QuestionBank directory not found at ${questionBankDir}`);
      return;
    }
    ...
    const jsonFiles = getAllJsonFiles(questionBankDir);
    console.log(`[Seeding] Found ${jsonFiles.length} JSON files in QuestionBank.`);
    ...
    for (const filePath of jsonFiles) {
      ...
      try {
        ...
        console.error(`[Seeding] Failed to parse JSON file ${filename}:`, err);
        continue;
      }

      const questionsArray = Array.isArray(parsed) ? parsed : (parsed.questions || []);
      if (questionsArray.length === 0) continue;

      const bulkOps = questionsArray.map((q, idx) => {
        const letter = q.correct_letter || q.correctAnswer || q.correct_answer || "";
        ...
        let questionText = q.question_text || q.q || q.question || "";
        if (questionText && questionText.startsWith("ParsedQuestion:")) {
          questionText = questionText.replace("ParsedQuestion:", "").trim();
        }

        // Infer exam and sub types
        const fileLower = filename.toLowerCase();
        let mappedExamType = "General";
        let mappedSubType = "General";

        if (fileLower.includes("rrb")) {
          mappedExamType = "RRB";
          mappedSubType = "RRB GD";
        } else if (fileLower.includes("ssc")) {
          mappedExamType = "SSC";
          mappedSubType = "SSC GD";
        } else if (fileLower.includes("sbi") || fileLower.includes("ibps")) {
          mappedExamType = "Banking";
          mappedSubType = "SBI Clerk";
        }

        const uniqueId = q.unique_id || q.id || `q_${filename}_${idx}_${Date.now()}`;
    ...
    console.log(`[Seeding] Successfully imported ${totalImported} questions into the local MongoDB!`);
  } catch (err) {
    console.error("[Seeding] Error seeding QuestionBank questions:", err);
  }
};

mongoose.connect(process.env.MONGODB_URI || "mongodb://127.0.0.1:27017/kr-academy")
  .then(() => {
...
// Startup validation: Warn if BREVO_API_KEY or EMAIL_FROM is missing
if (!process.env.BREVO_API_KEY || !process.env.EMAIL_FROM) {
  console.warn("=======================================================================");
  console.warn("WARNING: BREVO_API_KEY or EMAIL_FROM environment variable is missing.");
  console.warn("Email functionality (OTP sending, password reset) will run in offline demo mode.");
  console.warn("=======================================================================");
  if (!process.env.EMAIL_FROM) process.env.EMAIL_FROM = "no-reply@kr-institute.in";
  if (!process.env.BREVO_API_KEY) process.env.BREVO_API_KEY = "mock_key";
}
...
const sendMail = async (to, subject, text, html) => {
  ...
  if (apiKey === "mock_key") {
    console.log("==========================================================");
    console.log(`[OFFLINE DEMO EMAIL] To: ${to}`);
    console.log(`[OFFLINE DEMO EMAIL] Subject: ${subject}`);
    console.log(`[OFFLINE DEMO EMAIL] Text Content: ${text}`);
    console.log("==========================================================");
    return { success: true, messageId: "offline_mock_id_" + Date.now() };
  }
...
const generateMocksForCourse = async (courseId, courseTitle, category = "Bank & Insurance", sectionName = null) => {
  ...
  // Map courseId to db course field patterns
  let courseDbFilter = null;
  if (courseId) {
    const cleanId = courseId.toLowerCase().trim();
    if (cleanId === "ssc_chsl" || cleanId.includes("ssc_chsl")) {
      courseDbFilter = "SSC CHSL";
    } else if (cleanId === "ssc_cgl" || cleanId.includes("ssc_cgl")) {
      courseDbFilter = "SSC CGL";
    } else if (cleanId === "sbi_clerk" || cleanId.includes("sbi_clerk")) {
      courseDbFilter = "SBI Clerk Prelims";
    } else if (cleanId === "ibps_clerk" || cleanId.includes("ibps_clerk")) {
      courseDbFilter = "IBPS Clerk Prelims";
    }
  }
...
const getMockEligibleQuestions = async (exam_type, sub_type, sectionName = null) => {
  ...
  // Map sub_type parameter to clean display title supporting mocks, shuffled IDs, and pattern modules
...
app.get("/api/admin/questions", verifyAdmin, async (req, res) => {
  try {
    ...
    if (exam_type) {
      // Handle flexible aliases (e.g. "SSC" matches "SSC Exams" or "ssc_gd")
      if (exam_type.toUpperCase() === "SSC") {
        filter.$or = [
          { exam_type: { $regex: "ssc", $options: "i" } },
          { category: { $regex: "ssc", $options: "i" } }
        ];
      } else if (exam_type.toUpperCase() === "RRB") {
        filter.$or = [
          { exam_type: { $regex: "rrb", $options: "i" } },
          { category: { $regex: "rrb", $options: "i" } }
        ];
      } else if (exam_type.toUpperCase() === "BANKING") {
        filter.$or = [
          { exam_type: { $regex: "bank", $options: "i" } },
          { category: { $regex: "bank", $options: "i" } }
        ];
      } else {
        filter.$or = [
          { exam_type: { $regex: exam_type, $options: "i" } },
          { category: { $regex: exam_type, $options: "i" } }
        ];
      }
    }
    
    if (sub_type) {
      const regexVal = { $regex: sub_type, $options: "i" };
      const subTypeFilter = [
        { sub_type: regexVal },
        { paper_name: regexVal },
        { course: regexVal },
        { source_file: regexVal }
      ];
      
      if (filter.$or) {
        // If we already have exam_type filter, intersect them using $and
        filter.$and = [
          { $or: filter.$or },
          { $or: subTypeFilter }
        ];
...
// Admin API: List all distinct source files/papers in Question pool
app.get("/api/admin/questions/sources", verifyAdmin, async (req, res) => {
  try {
    const sources = await Question.distinct("source_file");
    const subTypes = await Question.distinct("sub_type");
    const paperNames = await Question.distinct("paper_name");
    const courses = await Question.distinct("course");

    // Gather, deduplicate, filter, and sort
    const mergedList = [...new Set([
      ...sources.filter(Boolean),
      ...subTypes.filter(Boolean),
      ...paperNames.filter(Boolean),
      ...courses.filter(Boolean)
    ])].sort();

    res.json({ success: true, sources: mergedList });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
...
app.put("/api/admin/questions/:unique_id", verifyAdmin, async (req, res) => {
  ...
  try {
    ...
    if (question !== undefined) {
      ...
      updateFields.q = question; // Sync legacy field
    }
    ...
    if (correctOption !== undefined) {
      ...
      const cleanLetter = correctOption.trim().toLowerCase();
      let correctIndex = -1;
      if (cleanLetter === "a") correctIndex = 0;
      else if (cleanLetter === "b") correctIndex = 1;
      else if (cleanLetter === "c") correctIndex = 2;
      ...
      if (correctIndex !== -1) {
        updateFields.correct = correctIndex; // Sync legacy correct index field
      }
    }
