...
// Temporary debug route to trigger and monitor CGL image migration
app.get("/api/run-image-migration", async (req, res) => {
  try {
    const fs = require("fs");
    const path = require("path");
    ...
    if (!fs.existsSync(jsonDir)) {
      return res.status(404).json({ error: `JSON directory not found at ${jsonDir}` });
    }
    ...
    const logs = [];
    logs.push(`Found ${jsonFiles.length} JSON files to scan.`);
    ...
    for (const filePath of jsonFiles) {
      ...
      try {
        ...
        logs.push(`Error parsing ${filename}: ${e.message}`);
        continue;
      }
      ...
      if (bulkOps.length > 0) {
        ...
        logs.push(`Processed ${filename}: Queued ${bulkOps.length} updates. Modified ${result.modifiedCount} docs.`);
      }
    }
    
    return res.json({ success: true, totalUpdated, logs });
  } catch (err) {
